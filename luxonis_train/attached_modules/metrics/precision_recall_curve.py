...
class PrecisionRecallCurve(BaseMetric):
    ...
    Metadata:
        - Module type: metric
        - Registry name: ``PrecisionRecallCurve``
        - Task: BOUNDINGBOX, INSTANCE_KEYPOINTS, INSTANCE_SEGMENTATION
        - Attached node types: ``BaseDetectionHead``
        - Inputs: ``detections_pre_nms``, ``target_boundingbox``
        - Outputs: dictionary with ``confidence``, ``precision``, ``recall``,
          ``f1``, ``max_f1``, and ``confidence_at_max_f1``
        - State: ``true_positives``, ``false_positives``, ``target_count``

    Prediction format:
        ``detections_pre_nms`` holds the decoded candidates of the attached
        head, before NMS. The metric asks the head for them, then runs NMS
        itself so that one decode serves the whole confidence grid.
    ...
    Provenance:
        ...
        - Implementation notes: Reports the best F1 over the grid and the
          confidence that reaches it.
