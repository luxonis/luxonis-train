...
class ClosestIsPositiveAccuracy(BaseMetric):
    ...
    References:
        - Source: Adapted from a TensorFlow triplet-loss example.
        - License: Apache-2.0 (this project)

    Notes:
        Optionally accumulates embeddings across batches using the
        attached node's cross-batch memory size.
...
class MedianDistances(BaseMetric):
    r"""Median embedding distance diagnostics.
    ...
    Outputs:
        - ``all``, ``closest``, ``positive``, ``closest_vs_positive``
          (``Tensor``): scalar medians
    ...
    Notes:
        Returns NaN-valued diagnostics when no distances have been
        accumulated.
