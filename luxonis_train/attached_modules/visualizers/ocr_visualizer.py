...
class OCRVisualizer(BaseVisualizer):
    r"""Visualize OCR predictions and optional text targets.
    ...
    Inputs:
        ...
        - ``targets`` (``Tensor | None``): :math:`\left[B,
          T_{max}\right]` character codes, ``0``-padded

    Outputs:
        - ``tuple[Tensor, Tensor]``: :math:`\left[B, 3, H, W\right]`,
          the overlay and a predictions-versus-targets panel
    ...
    Notes:
        Decodes predictions with the attached ``OCRCTCHead`` decoder and
