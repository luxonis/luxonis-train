...
def hubai_export(
    ...
    """Convert an ONNX NNArchive using HubAI SDK.

    If a model with the given name already exists on HubAI, a new variant will
    be created under that model. Otherwise, a new model will be created.
...
def _resolve_hubai_model(
    ...
    Returns:
        ``tuple[str, str | None]``: The model to put the variant under, and the
            model this call created. The second value is ``None`` if an existing
            model was used, so only a new model is cleaned up on failure.
