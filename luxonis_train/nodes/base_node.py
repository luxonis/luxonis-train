...
class BaseNode(nn.Module, VariantBase, register=False, registry=NODES):
    ...
    def get_attached(self, value: list[T] | T) -> list[T] | T:
        ...
        Args:
            value (list[T] | T): List to get the attached elements from. Can be either a list of tensors or a list of sizes.

        Returns:
            list[T] | T: Attached elements. If ``attach_index`` is set to ``"all"`` or is a slice, returns a list of attached elements.
