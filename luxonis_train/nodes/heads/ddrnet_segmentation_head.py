...
class DDRNetSegmentationHead(BaseHead):
    ...
    Inputs:
        - ``inputs`` (``Tensor``): :math:`\left[B, C, H/8, W/8\right]`
    ...
    Outputs:
        ...
        - export:
          ...
          - ``Tensor``: :math:`\left[B, H, W\right]` ``int32`` class
            indices, or :math:`\left[B, 1, H, W\right]` when ``n_classes
            == 1``
    ...
    Notes:
        Applies batch normalization, convolutional projection, and
