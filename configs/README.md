...
| `strict_checkpoint_loading` | `bool` | `False`       | Require checkpoint loading to match the model state dict exactly; if strict loading fails, raise an error |
...
| `disable_balanced_log_images` | `bool`                                         | `False`                                | Disable class-balanced sampling when logging classification visualizations and log the first `n_log_images` sequentially instead                 |
